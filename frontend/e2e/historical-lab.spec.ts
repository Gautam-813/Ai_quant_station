...
test.describe('Historical Lab Page', () => {
  test.beforeEach(async ({ page }) => {
    await login(page)
  })

  test('loads Historical Lab page with mode tabs', async ({ page }) => {
    await page.getByRole('link', { name: /Historical Lab/ }).click()
    await page.waitForURL('**/historical-lab')
    await expect(page.getByRole('heading', { name: /Historical Lab/ })).toBeVisible()
  })
})
